...
# define folders
cwd = '/home/users/junhokang/Projects/02_MOGAD/star_mapped'
...
def read_process(adata,version,
    ...
    if write:
        ...
        adata.write('%s%s_filtered.h5ad'%(version,sample))
#        sc.write('%s%s_filtered'%(version,sample),adata)
    return adata

def read_process1(adata,version,
                 species = 'human',
                 sample = None,
                 define_var = True,
                 min_n_counts = 20,
...
def read_process2(adata,version,
                 sample = None,
                 call_doublet = True,
                 write = True,
                 min_n_counts = 1000,
...
def us(adata,gene,groups=None, show=False, exclude =None,figsize=None,cmap='OrRd',**kwargs):
    ...
    if ',' in gene:
        gene = gene.split(',')
    if groups:
        sc.pl.umap(adata,color=gene,color_map=cmap,groups=groups, show=show, **kwargs)
    else:
        if exclude: # list to not show
            groups = [x for x in set(adata.obs[gene]) if x not in exclude]
            sc.pl.umap(adata,color=gene,color_map=cmap,groups=groups, show=show, **kwargs)
        else:
            sc.pl.umap(adata,color=gene,color_map=cmap,show=show, **kwargs)
...
def save_fig(version,figcount,fig_format='pdf',fig_folder='11_Figs',dpi=300):
    
    plt.savefig('%s/%s%s.%s'%(fig_folder,version,figcount,fig_format),bbox_inches='tight',format=fig_format,dpi=dpi)
